...
namespace FMODUnity
{
    ...
    public static class RuntimeUtils
    {
        ...
        public static FMOD.ATTRIBUTES_3D To3DAttributes(Vector3 position, Vector3 velocity, Vector3 forward, Vector3 up)
        {
            FMOD.ATTRIBUTES_3D attributes = new FMOD.ATTRIBUTES_3D();
        ...
        private static readonly string BankUnzipPath = PathEx.GetUnzippedPath();
        private static readonly string BankHotUpdatePath = PathEx.GetHotUpdatePath();
        internal static string GetBankPath(string bankName)
        {
#if !UNITY_EDITOR
            string bankFilePersisitentPath;
            string ext = Path.GetExtension(bankName);
            // 首先检查沙盒目录下是否存在Bank
            if (ext != BankExtension)
            {
                bankFilePersisitentPath = Path.Combine(BankHotUpdatePath, $"{bankName}{BankExtension}");
            }
            else
            {
                bankFilePersisitentPath = Path.Combine(BankHotUpdatePath, bankName);
            }
            if (File.Exists(bankFilePersisitentPath))
            {
                return bankFilePersisitentPath;
            }

            // 再检查unzip目录下是否存在Bank
            if (ext != BankExtension)
            {
                bankFilePersisitentPath = Path.Combine(BankUnzipPath, $"{bankName}{BankExtension}");
            }
            else
            {
                bankFilePersisitentPath = Path.Combine(BankUnzipPath, bankName);
            }
            if (File.Exists(bankFilePersisitentPath))
            {
                 return bankFilePersisitentPath;
            ...
            var hasPlatform = Settings.Instance.HasPlatforms;
            var platformFolder = Settings.Instance.GetBankPlatform(FMODPlatform.PlayInEditor);
            if (hasPlatform)
            {
                bankFolder = global::System.IO.Path.Combine(bankFolder, platformFolder);
            }
            // 区分地区
            if (!MGameContext.singleton.IsMainChannel)
            {
                var overseaBankPath = PathEx.GetChannelArtPathEx(MGameContext.singleton.CurrentChannel, "_FMod");
                if (hasPlatform)
                {
                    overseaBankPath = DirectoryEx.Combine(overseaBankPath, platformFolder);
                }

                if (Directory.Exists(overseaBankPath) && Directory.GetFiles(overseaBankPath, "*.bank").Length > 0)
                {
                    bankFolder = overseaBankPath;
                }
            }
