...
retry:
        ...
        void OnApplicationPause(bool pauseStatus)
        {
            if (studioSystem.isValid())
            {
                #region 这里将事件的暂停状态改为通过MAudioMgr管理器管理
                var eventStatus = pauseStatus;
                var bridge = MInterfaceMgr.singleton.GetInterface<IMoonClientBridge>("MoonClientBridge");
                if (bridge != null)
                {
                    eventStatus = bridge.IsAudioPaused;
                }
                #endregion
                PauseAllEvents(eventStatus);
        ...
        public static void LoadBank(string bankName, bool loadSamples = false)
        {
            ...
            else
            {
                ...
                {
                    if (bankPath.StartsWith("jar:file://"))
                    {
        ...
        // ADD by ZZR
        public static Vector3 GetListenerLocation(int listenerIndex)
        {
            if (listenerIndex < 0 || listenerIndex >= HasListener.Length)
            {
                return Vector3.zero;
            }
            if (HasListener[listenerIndex])
            {
                Instance.studioSystem.getListenerAttributes(listenerIndex, out var attr);
                return new Vector3(attr.position.x, attr.position.y, attr.position.z);
            }
            return Vector3.zero;
        }
