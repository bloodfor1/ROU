...
public class Reporter : MonoBehaviour
{
...
#endif
		systemMemorySize = MoonCommonLib.MSystemInfoUtil.GetSystemMemorySize().ToString();
	...
	void DrawInfo()
	{
		...
		GUILayout.Label("Lua GC Memory " + luaTotalMemory.ToString("0.000") + " mb", nonStyle, GUILayout.Height(size.y));
	...
	void drawToolBar()
	{
		...
		if (GUILayout.Button("LuaFast", barStyle, GUILayout.Width(size.x * 2), GUILayout.Height(size.y * 2))) {
			currentView = ReportView.LuaFast;
		}
...
#region luafast
	...
	private List<string> hasAddLuaCodeList = new List<string>();
	private Vector2 luafastViewScrollPos = Vector2.zero;
	void drawLuaFastTool()
	{
		int halfScreenWidth = Screen.width/2;
		
		GUILayout.BeginHorizontal("Box");
		int toolBarComWidth = halfScreenWidth / 2;
		if (GUILayout.Button("添加一个LuaCode", barStyle, GUILayout.Width(toolBarComWidth), GUILayout.Height(size.y * 2)))
		{
			hasAddLuaCodeList.Add("");
			return;
		}
		if (GUILayout.Button(closeContent, barStyle, GUILayout.Width(toolBarComWidth), GUILayout.Height(size.y * 2)))
		{
			currentView = ReportView.Logs;
			return;
		}
		...
		GUILayout.EndHorizontal();
		luafastViewScrollPos = GUILayout.BeginScrollView(luafastViewScrollPos,"Box");
		luafastViewScrollPos.x = 0;
		int luaCodeListCount = hasAddLuaCodeList.Count;
		int fastCodeBtnWidth = halfScreenWidth / 4;
		float fastCodeInputTxtHeight = size.y*3;
		for (int i = 0; i < luaCodeListCount; i++)
		{
			GUILayout.BeginVertical();
			hasAddLuaCodeList[i] = GUILayout.TextArea(hasAddLuaCodeList[i],GUILayout.Width(halfScreenWidth),GUILayout.Height(fastCodeInputTxtHeight));
			GUILayout.BeginHorizontal();
			if (GUILayout.Button("执行luaCode", barStyle,GUILayout.Width(fastCodeBtnWidth), GUILayout.MinHeight(fastCodeInputTxtHeight)))
			{
				IMLua luaEngine = MInterfaceMgr.singleton.GetInterface<IMLua>(MCommonFunctions.GetHash("MLua"));
				if (luaEngine != null)
					luaEngine.DoString(hasAddLuaCodeList[i]);
			}
			if (GUILayout.Button("添加一条", barStyle, GUILayout.Width(fastCodeBtnWidth), GUILayout.Height(fastCodeInputTxtHeight)))
			{
				hasAddLuaCodeList.Insert(i,"");
				return;
			}
			if (GUILayout.Button("删除luaCode", barStyle, GUILayout.Width(fastCodeBtnWidth), GUILayout.Height(fastCodeInputTxtHeight)))
			{
				hasAddLuaCodeList.RemoveAt(i);
				return;
			}
			if (GUILayout.Button("清空内容", barStyle, GUILayout.Width(fastCodeBtnWidth), GUILayout.Height(fastCodeInputTxtHeight)))
			{
				hasAddLuaCodeList[i] = "";
			}
			GUILayout.EndHorizontal();
	...
	void DrawLogs()
	{
		...
		for (int i = startIndex; (startIndex + index) < endIndex; i++) {
			...
			float w = 0f;
			if (collapse)
				w = barStyle.CalcSize(tempContent).x + 20;
	...
	public void OnGUIDraw()
	{
		...
		else if (currentView == ReportView.LuaFast)
		{
			drawLuaFastTool();
		}
	...
	void Update()
	{
        fpsText = MyFPSDisplay.stringsFrom00To99[Mathf.Clamp(Mathf.CeilToInt(fps), 0, 99)];
		gcTotalMemory = (((float)System.GC.GetTotalMemory(false)) / 1024 / 1024);
        //if (MoonClient.MGame.singleton.LuaEngine != null)
        //{
        //    luaTotalMemory = MoonClient.MGame.singleton.LuaEngine.GetMemorySize();
        //}
	...
	void AddLog(string condition, string stacktrace, LogType type)
	{
		...
		if (logsDic.ContainsKey(_condition, stacktrace)) {
            ...
            if(!Application.isEditor || type != LogType.Log)
            {
                var count = logsDic[_condition][stacktrace].count;
                if (count >= 0 && (count % 30) == 0)
                {
                    Debug.LogWarningFormat("--repeatLogTimes[{0}]\n{1}\n{2}", count, _condition, stacktrace);
                }
            }
		}
		else {
	...
	void CaptureLogThread(string condition, string stacktrace, LogType type)
	{
        ...
        try
        {
            ...
            lock (threadedLogs)
            {
                logWriter?.Write($"[{log.logType}]{log.condition}\n{log.stacktrace}");
    ...
    private  static string logPath => Path.Combine(MoonCommonLib.PathEx.GetCachePath(), $"Log/log {DateTime.Now:yy-MM-dd HHmmss}.log");
