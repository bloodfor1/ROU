...
namespace RenderHeads.Media.AVProVideo
{
	...
	public class MediaPlayer : MonoBehaviour
	{
		...
		public EFileLocation m_VideoLocation = EFileLocation.RelativeToStreamingAssetsFolder;
		...
		[SerializeField]
		private EFileLocation m_SubtitleLocation = EFileLocation.RelativeToStreamingAssetsFolder;
		private EFileLocation m_queueSubtitleLocation;
		...
		public class PlatformOptions
		{
			...
			public EFileLocation pathLocation = EFileLocation.RelativeToStreamingAssetsFolder;
			public string path;
		...
		public bool OpenVideoFromFile(string path)
        {
			return OpenVideoFromFile(m_VideoLocation, path, true);
		}
		...
		{
			m_VideoLocation = EFileLocation.AbsolutePathOrURL;
			m_VideoPath = "buffer";
			m_AutoStart = autoPlay;
		...
		public EFileLocation SubtitleLocation
		{
		...
		private IEnumerator LoadSubtitlesCoroutine(string url, EFileLocation fileLocation, string filePath)
		{
		...
		public void CloseVideo()
		{
			...
			if (m_Control != null)
			{
				if (m_events != null && m_VideoOpened && m_events.HasListeners() && IsHandleEvent(MoonCommonLib.EEventType.Closing))
				{
					m_events.Invoke(this, MoonCommonLib.EEventType.Closing, EErrorCode.None);
				}
...
#region Platform and Path
	...
	// Setup for running builds
		...
		public static string GetPath(EFileLocation location)
		{
			string result = string.Empty;
			switch (location)
			{
				case EFileLocation.AbsolutePathOrURL:
					break;
				case EFileLocation.RelativeToDataFolder:
					result = Application.dataPath;
					break;
				case EFileLocation.RelativeToPersistentDataFolder:
					result = Application.persistentDataPath;
					break;
				case EFileLocation.RelativeToProjectFolder:
#if !UNITY_WINRT_8_1
					string path = "..";
...
#endif
					break;
				case EFileLocation.RelativeToStreamingAssetsFolder:
		...
		public static string GetFilePath(string path, EFileLocation location)
		{
			string result = string.Empty;
		...
		private string GetPlatformFilePath(Platform platform, ref string filePath, ref EFileLocation fileLocation)
		{
			string result = string.Empty;
...
#region Support for Time Scale
		...
		{
			EErrorCode errorCode = m_Control.GetLastError();
			if (EErrorCode.None != errorCode)
			{
				Debug.LogError("[AVProVideo] Error: " + Helper.GetErrorMessage(errorCode));

				if (m_events != null && m_events.HasListeners() && IsHandleEvent(MoonCommonLib.EEventType.Error))
				{
					m_events.Invoke(this, MoonCommonLib.EEventType.Error, errorCode);
				}
			}
		}
		...
		private void UpdateEvents()
		{
			if (m_events != null && m_Control != null && m_events.HasListeners())
			{
				...
				if (IsHandleEvent(MoonCommonLib.EEventType.FinishedPlaying))
				{
					if (FireEventIfPossible(MoonCommonLib.EEventType.FinishedPlaying, m_EventFired_FinishedPlaying))
					{
				...
				// Reset some event states that can reset during playback
				{
					...
					if (m_EventFired_Started && IsHandleEvent(MoonCommonLib.EEventType.Started) &&
					...
					if (m_EventFired_FinishedPlaying && IsHandleEvent(MoonCommonLib.EEventType.FinishedPlaying) &&
				...
				m_EventFired_MetaDataReady = FireEventIfPossible(MoonCommonLib.EEventType.MetaDataReady, m_EventFired_MetaDataReady);
				m_EventFired_ReadyToPlay = FireEventIfPossible(MoonCommonLib.EEventType.ReadyToPlay, m_EventFired_ReadyToPlay);
				m_EventFired_Started = FireEventIfPossible(MoonCommonLib.EEventType.Started, m_EventFired_Started);
				m_EventFired_FirstFrameReady = FireEventIfPossible(MoonCommonLib.EEventType.FirstFrameReady, m_EventFired_FirstFrameReady);

				// Events that can fire multiple times
				{
					// Subtitle changing
					if (FireEventIfPossible(MoonCommonLib.EEventType.SubtitleChange, false))
					{
						m_previousSubtitleIndex = m_Subtitles.GetSubtitleIndex();
					}

					// Resolution changing
					if (FireEventIfPossible(MoonCommonLib.EEventType.ResolutionChanged, false))
					{
					...
					// Stalling
					if (IsHandleEvent(MoonCommonLib.EEventType.Stalled))
					{
						...
						if (newState != m_EventState_PlaybackStalled)
						{
							...
							var newEvent = m_EventState_PlaybackStalled ? MoonCommonLib.EEventType.Stalled : MoonCommonLib.EEventType.Unstalled;
							FireEventIfPossible(newEvent, false);
						}
					}
					// Seeking
					if (IsHandleEvent(MoonCommonLib.EEventType.StartedSeeking))
					{
						...
						if (newState != m_EventState_PlaybackSeeking)
						{
							...
							var newEvent = m_EventState_PlaybackSeeking ? MoonCommonLib.EEventType.StartedSeeking : MoonCommonLib.EEventType.FinishedSeeking;
							FireEventIfPossible(newEvent, false);
						}
					}
					// Buffering
					if (IsHandleEvent(MoonCommonLib.EEventType.StartedBuffering))
					{
						...
						if (newState != m_EventState_PlaybackBuffering)
						{
							...
							var newEvent = m_EventState_PlaybackBuffering ? MoonCommonLib.EEventType.StartedBuffering : MoonCommonLib.EEventType.FinishedBuffering;
							FireEventIfPossible(newEvent, false);
						}
					}
				}
		...
		protected bool IsHandleEvent(MoonCommonLib.EEventType eventType)
		{
			return ((uint)m_eventMask & (1 << (int)eventType)) != 0;
		}

		private bool FireEventIfPossible(MoonCommonLib.EEventType eventType, bool hasFired)
		{
			if (CanFireEvent(eventType, hasFired))
			{
				hasFired = true;
				m_events.Invoke(this, eventType, EErrorCode.None);
			}
			return hasFired;
		}

		private bool CanFireEvent(MoonCommonLib.EEventType et, bool hasFired)
		{
			bool result = false;
			if (m_events != null && m_Control != null && !hasFired && IsHandleEvent(et))
			{
				switch (et)
				{
					case MoonCommonLib.EEventType.FinishedPlaying:
						...
						result = (!m_Control.IsLooping() && m_Control.CanPlay() && m_Control.IsFinished())
							...
							;
						break;
					case MoonCommonLib.EEventType.MetaDataReady:
						result = (m_Control.HasMetaData());
						break;
					case MoonCommonLib.EEventType.FirstFrameReady:
						result = (m_Texture != null && m_Control.CanPlay() && m_Control.HasMetaData() && m_Texture.GetTextureFrameCount() > 0);
						break;
					case MoonCommonLib.EEventType.ReadyToPlay:
						result = (!m_Control.IsPlaying() && m_Control.CanPlay() && !m_AutoStart);
						break;
					case MoonCommonLib.EEventType.Started:
						result = (m_Control.IsPlaying());
						break;
					case MoonCommonLib.EEventType.SubtitleChange:
						result = (m_previousSubtitleIndex != m_Subtitles.GetSubtitleIndex());
						break;
					case MoonCommonLib.EEventType.Stalled:
						result = m_Info.IsPlaybackStalled();
						break;
					case MoonCommonLib.EEventType.Unstalled:
						result = !m_Info.IsPlaybackStalled();
						break;
					case MoonCommonLib.EEventType.StartedSeeking:
						result = m_Control.IsSeeking();
						break;
					case MoonCommonLib.EEventType.FinishedSeeking:
						result = !m_Control.IsSeeking();
						break;
					case MoonCommonLib.EEventType.StartedBuffering:
						result = m_Control.IsBuffering();
						break;
					case MoonCommonLib.EEventType.FinishedBuffering:
						result = !m_Control.IsBuffering();
						break;
					case MoonCommonLib.EEventType.ResolutionChanged:
