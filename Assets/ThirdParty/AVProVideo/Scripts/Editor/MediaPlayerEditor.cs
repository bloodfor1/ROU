...
namespace RenderHeads.Media.AVProVideo.Editor
{
	...
	public class MediaPlayerEditor : UnityEditor.Editor
	{
		...
		public override void OnInspectorGUI()
		{
			...
			// Display the file name and buttons to load new files
			{
				...
				//if (!Application.isPlaying)
				{
					...
					{
						string startFolder = GetStartFolder(_propPath.stringValue, (MoonCommonLib.EFileLocation)_propLocation.enumValueIndex);
						string videoPath = media.m_VideoPath;
						string fullPath = string.Empty;
						MoonCommonLib.EFileLocation fileLocation = media.m_VideoLocation;
					...
					ShowFileWarningMessages(_propPath.stringValue, (MoonCommonLib.EFileLocation)_propLocation.enumValueIndex, media.m_AutoOpen, Platform.Unknown);
					GUI.color = Color.white;
				}
		...
		void RecentMenuCallback_Select(object obj)
		{
			...
			MoonCommonLib.EFileLocation fileLocation = MoonCommonLib.EFileLocation.RelativeToStreamingAssetsFolder;
		...
		private static void ShowFileWarningMessages(string filePath, MoonCommonLib.EFileLocation fileLocation, bool isAutoOpen, Platform platform)
		{
			...
			else
			{
				...
				if (finalPath.Contains("://"))
				{
					...
					if (fileLocation != MoonCommonLib.EFileLocation.AbsolutePathOrURL)
					{
				...
				{
					if (fileLocation != MoonCommonLib.EFileLocation.AbsolutePathOrURL && filePath.StartsWith("/"))
					{
			...
			if (fileLocation == MoonCommonLib.EFileLocation.RelativeToStreamingAssetsFolder)
			{
				if (!System.IO.Directory.Exists(Application.streamingAssetsPath))
				{
					GUILayout.BeginHorizontal();
		...
		private void OnInspectorGUI_Events()
		{
			...
			if (_expandEvents)
			{
				...
				_propEventMask.intValue = EditorGUILayout.MaskField("Triggered Events", _propEventMask.intValue, System.Enum.GetNames(typeof(MoonCommonLib.EEventType)));
			...
			if (_expandSubtitles)
			{
				...
				EditorGUILayout.PropertyField(_propSubtitles, new GUIContent("Load External Subtitles"));
				{
					...
					//if (!Application.isPlaying)
					{
						...
						if (GUILayout.Button("BROWSE"))
						{
							string startFolder = GetStartFolder(_propSubtitlePath.stringValue, (MoonCommonLib.EFileLocation)_propSubtitleLocation.enumValueIndex);
							string videoPath = media.SubtitlePath;
							string fullPath = string.Empty;
							MoonCommonLib.EFileLocation fileLocation = media.SubtitleLocation;
						...
						ShowFileWarningMessages(_propSubtitlePath.stringValue, (MoonCommonLib.EFileLocation)_propSubtitleLocation.enumValueIndex, media.m_AutoOpen, Platform.Unknown);
						GUI.color = Color.white;
					}
					...
					if (Application.isPlaying)
					{
						...
						if (GUILayout.Button("Load"))
						{
							media.EnableSubtitles((MoonCommonLib.EFileLocation)_propSubtitleLocation.enumValueIndex, _propSubtitlePath.stringValue);
						}
		...
		private static string GetStartFolder(string path, MoonCommonLib.EFileLocation fileLocation)
		{
		...
		private void GUI_OverridePath(int platformIndex)
		{
			...
			if (GUILayout.Button("BROWSE"))
			{
				...
				MoonCommonLib.EFileLocation fileLocation = MoonCommonLib.EFileLocation.RelativeToStreamingAssetsFolder;
				string startFolder = GetStartFolder(propPath.stringValue, (MoonCommonLib.EFileLocation)propLocation.enumValueIndex);
				string fullPath = string.Empty;
			...
			if (GUI.enabled)
			{
				ShowFileWarningMessages(propPath.stringValue, (MoonCommonLib.EFileLocation)propLocation.enumValueIndex, false, (Platform)platformIndex);
			}
		}
		...
		private static bool Browse(string startPath, ref string filePath, ref MoonCommonLib.EFileLocation fileLocation, ref string fullPath, string extensions)
		{
			bool result = false;
		...
		private static void GetRelativeLocationFromPath(string path, ref string filePath, ref MoonCommonLib.EFileLocation fileLocation)
		{
			...
			if (path.StartsWith(projectRoot))
			{
				if (path.StartsWith(Application.streamingAssetsPath))
				{
					...
					fileLocation = MoonCommonLib.EFileLocation.RelativeToStreamingAssetsFolder;
				}
				else if (path.StartsWith(Application.dataPath))
				{
					...
					fileLocation = MoonCommonLib.EFileLocation.RelativeToDataFolder;
				}
				else
				{
					// Must be project relative path
					filePath = GetPathRelativeTo(projectRoot, path);
					fileLocation = MoonCommonLib.EFileLocation.RelativeToProjectFolder;
				}
			}
			else
			{
				...
				if (path.StartsWith(Application.persistentDataPath))
				{
					...
					fileLocation = MoonCommonLib.EFileLocation.RelativeToPersistentDataFolder;
				}

				// Must be absolute path
				filePath = path;
				fileLocation = MoonCommonLib.EFileLocation.AbsolutePathOrURL;
			}
		}
	}
}