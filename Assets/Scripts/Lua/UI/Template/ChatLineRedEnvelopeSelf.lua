...
--lua class define
---@class ChatLineRedEnvelopeSelfParameter
...
---@class ChatLineRedEnvelopeSelf : BaseUITemplate
---@field Parameter ChatLineRedEnvelopeSelfParameter

ChatLineRedEnvelopeSelf = class("ChatLineRedEnvelopeSelf", super)
--lua class define end

--lua functions
function ChatLineRedEnvelopeSelf:Init()

    super.Init(self)

end --func end
--next--
function ChatLineRedEnvelopeSelf:OnDestroy()

    self.head2d = nil

end --func end
--next--
function ChatLineRedEnvelopeSelf:OnDeActive()

    -- do nothing

end --func end
--next--
function ChatLineRedEnvelopeSelf:OnSetData(data)
    ...
    if MgrMgr:GetMgr("GuildMgr").IsSelfHasGuild() and
            l_guildData.GetSelfGuildPosition() <= l_guildData.EPositionType.Beauty and
            self.msgPack.channel == DataMgr:GetData("ChatData").EChannel.GuildChat then
        self.Parameter.Permission:SetActiveEx(true)
        local l_permissionStr = l_guildData.GetPositionName(l_guildData.GetSelfGuildPosition()) or ""
        l_permissionStr = StringEx.Format("[{0}]", l_permissionStr)
        self.Parameter.Permission.LabText = l_permissionStr
        --公会团宠颜色显示不一样
        if l_guildData.IsGuildBeauty(l_guildData.GetSelfGuildPosition()) then
...
--next--
function ChatLineRedEnvelopeSelf:BindEvents()
...
--lua custom scripts
--- 更新角色头像数据
function ChatLineRedEnvelopeSelf:_updatePlayerHead()
...
--- 设置聊天标签
function ChatLineRedEnvelopeSelf:_setCustomData()
...
return ChatLineRedEnvelopeSelf