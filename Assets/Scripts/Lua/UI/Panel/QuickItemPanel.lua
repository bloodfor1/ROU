--this file is gen by script
--you can edit this file in custom part


--lua model
module("UI", package.seeall)
QuickItemPanel = {}

--lua model end

--lua functions
---@class QuickItemPanel.QuickUseTemplate
---@field PanelRef MoonClient.MLuaUIPanel
---@field TxtQuick MoonClient.MLuaUICom[]
---@field TxtCd MoonClient.MLuaUICom[]
---@field Offset MoonClient.MLuaUICom
---@field ImgQuick MoonClient.MLuaUICom[]
---@field ImgCd MoonClient.MLuaUICom[]
---@field BtnQuick MoonClient.MLuaUICom[]
---@field Add MoonClient.MLuaUICom[]

---@class QuickItemPanel
---@field PanelRef MoonClient.MLuaUIPanel
---@field PageView MoonClient.MLuaUICom
---@field PageMark MoonClient.MLuaUICom[]
---@field MarkBg MoonClient.MLuaUICom
---@field Content MoonClient.MLuaUICom
---@field QuickUseTemplate QuickItemPanel.QuickUseTemplate

---@return QuickItemPanel
function QuickItemPanel.Bind(ctrl)

	--dont override this function
	---@type MoonClient.MLuaUIPanel
	local panelRef = ctrl.uObj:GetComponent("MLuaUIPanel")
	ctrl:OnBindPanel(panelRef)
	return BindMLuaPanel(panelRef)
	
end --func end
--next--
--lua functions end

--lua custom scripts

--lua custom scripts end
return UI.QuickItemPanel