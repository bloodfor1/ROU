--this file is gen by script
--you can edit this file in custom part


--lua model
module("UI", package.seeall)
ReBackLetterPanel = {}

--lua model end

--lua functions
---@class ReBackLetterPanel
---@field PanelRef MoonClient.MLuaUIPanel
---@field Title MoonClient.MLuaUICom
---@field SkeletonGraphic MoonClient.MLuaUICom
---@field Open MoonClient.MLuaUICom
---@field Effect MoonClient.MLuaUICom[]
---@field Content MoonClient.MLuaUICom
---@field Close MoonClient.MLuaUICom
---@field BtnOpen MoonClient.MLuaUICom
---@field BtnFinish MoonClient.MLuaUICom
---@field BtnCloseNext MoonClient.MLuaUICom

---@return ReBackLetterPanel
---@param ctrl UIBase
function ReBackLetterPanel.Bind(ctrl)
	
	--dont override this function
	---@type MoonClient.MLuaUIPanel
	local panelRef = ctrl.uObj:GetComponent("MLuaUIPanel")
	ctrl:OnBindPanel(panelRef)
	return BindMLuaPanel(panelRef)
	
end --func end
--next--
--lua functions end

--lua custom scripts

--lua custom scripts end
return UI.ReBackLetterPanel