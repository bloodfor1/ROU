--this file is gen by script
--you can edit this file in custom part


--lua model
module("UI", package.seeall)
FastmountingPanel = {}

--lua model end

--lua functions
---@class FastmountingPanel.StallBagItemTemplate
---@field PanelRef MoonClient.MLuaUIPanel
---@field Selected MoonClient.MLuaUICom
---@field ItemNumber MoonClient.MLuaUICom
---@field ItemName MoonClient.MLuaUICom
---@field ItemIcon MoonClient.MLuaUICom
---@field ItemCount MoonClient.MLuaUICom
---@field ItemButton MoonClient.MLuaUICom

---@class FastmountingPanel
---@field PanelRef MoonClient.MLuaUIPanel
---@field rightScrollView MoonClient.MLuaUICom
---@field rightEmpty MoonClient.MLuaUICom
---@field leftPanel MoonClient.MLuaUICom
---@field leftItemIcon MoonClient.MLuaUICom
---@field leftItem MoonClient.MLuaUICom
---@field leftEmpty MoonClient.MLuaUICom
---@field itemName MoonClient.MLuaUICom
---@field infoPanel MoonClient.MLuaUICom
---@field fastUpBtn MoonClient.MLuaUICom
---@field emptyText MoonClient.MLuaUICom
---@field CloseBtn MoonClient.MLuaUICom
---@field BtnPrayText MoonClient.MLuaUICom
---@field StallBagItemTemplate FastmountingPanel.StallBagItemTemplate

---@return FastmountingPanel
function FastmountingPanel.Bind(ctrl)

	--dont override this function
	---@type MoonClient.MLuaUIPanel
	local panelRef = ctrl.uObj:GetComponent("MLuaUIPanel")
	ctrl:OnBindPanel(panelRef)
	return BindMLuaPanel(panelRef)
	
end --func end
--next--
--lua functions end

--lua custom scripts

--lua custom scripts end
return UI.FastmountingPanel